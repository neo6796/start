...
type MyOrder = { id: string; menuItemId: string };
...
export function MenuDayCard({
  ...
  closed: boolean;
  items: Item[];
  myOrdersByCategory: Record<string, MyOrder>;
}) {
    ...
    <section className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
        {Array.from(grouped.entries()).map(([category, list]) => {
          const myInCategory = myOrdersByCategory[category];
          return (
            <div key={category} className="px-4 py-3">
              <div className="mb-2 flex items-center justify-between">
                ...
                {myInCategory && (
                  <button
                    onClick={() => handleCancel(myInCategory.id)}
              ...
              <ul className="space-y-2">
                {list.map((item) => {
                  const selected = myInCategory?.menuItemId === item.id;
                  return (
                    <li
                      key={item.id}
                    ...
                    >
                      ...
                      <div className="flex flex-col items-end gap-2 whitespace-nowrap">
                        ...
                        {selected ? (
                          <span className="rounded-md bg-orange-600 px-3 py-1 text-xs font-medium text-white">
                            ✓ Vybrané
                          </span>
                        ) : (
                          <button
