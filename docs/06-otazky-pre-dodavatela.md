# Otázky pre dodávateľa stravy

Zavádzame aplikáciu na objednávanie obedov. Objednávky vám budú chodiť **automaticky e-mailom** vždy v rovnakom čase, v rovnakej podobe a s presnými počtami.

Aby sme ju nastavili podľa toho, ako fungujete vy, potrebujeme odpovede na tieto otázky. **Pre každého dodávateľa jeden vyplnený hárok** — aplikácia vie mať u každého iné nastavenia.

---

## Kontakt

- názov: ………
- kontaktná osoba: ………
- **e-mail na objednávky:** ……… *(môže byť aj viac, oddeľte čiarkou)*
- telefón: ………

---

## Ponuka

**Koľko jedál denne ponúkate?** ……… *(1 až 10)*

**Ako je jedlo zložené?**
- [ ] **komplet** — polievka aj dezert sú v cene, objednáva sa jedna položka
- [ ] **po častiach** — polievka alebo dezert sa objednávajú a účtujú zvlášť

**V ktoré dni varíte?** ………

**Máte dennú kapacitu?** Teda maximálny počet porcií, ktorý zvládnete? ………

**Ktoré dni nevaríte?** *(sviatky, celozávodná dovolenka, plánované odstávky)* ………

---

## Menu

**Ako a kedy nám posielate jedálny lístok na budúci týždeň?**
- [ ] e-mailom v tele správy
- [ ] e-mailom ako príloha — v akom formáte? *(Word, Excel, PDF, fotka)* ………
- [ ] papierom
- [ ] inak: ………

**V ktorý deň býva menu na budúci týždeň hotové?** ………

> Toto je dôležité: objednávky zbierame od pondelka a uzatvárame v piatok o 12:00. Ak menu príde až vo štvrtok, ľudia majú na výber len dva dni.

**Prosíme priložiť vzorku** — jeden jedálny lístok presne v podobe, v akej chodí. Podľa nej nastavíme zadávanie.

---

## Dovoz — kam a za akých podmienok

Časť našich ľudí pracuje počas týždňa na viacerých prevádzkach. Aplikácia vie objednávku **rozpísať podľa miest**, aby ste vedeli, koľko porcií ide kam — nielen koľko celkovo.

**Na ktoré miesta ste ochotní voziť?** Pre každé prosíme tri údaje:

| Miesto | Čas dovozu | Minimálny počet porcií |
|---|---|---|
| ……… | ……… | ……… |
| ……… | ……… | ……… |
| ……… | ……… | ……… |

**Minimum je dôležité.** Ak sa nejaký deň pod neho dostaneme, aplikácia nás na to upozorní **už pri týždennej uzávierke** — teda s časom na to, aby sme sa buď dohodli na výnimke, alebo tých pár ľudí presunuli inam. Nie ráno v deň obeda.

**Má niektoré miesto skorší termín na odhlásenie?** Napríklad ak auto vyráža skôr.
- miesto ……… → odhlásenie do ……… hod.

**Účtujete za dovoz na vzdialenejšie miesto príplatok?**
- [ ] nie, cena je rovnaká
- [ ] áno, ……… € *(za porciu / za dovoz — nehodiace sa škrtnite)*

---

## Termíny

**Dokedy sa dá odhlásiť z obeda na konkrétny deň?**
- [ ] v **deň obeda** do ……… hod.
- [ ] **predchádzajúci pracovný deň** do ……… hod.
- [ ] iné: ………

**Prijmete objednávku pridanú v deň obeda?** Napríklad keď sa niekto vráti z choroby a predák ho doobjedná.
- [ ] áno, do ……… hod.
- [ ] nie

> Ak áno, budeme vám v tom čase posielať **krátku korekciu** v tvare: *„pôvodne 47, storná 3, doobjednávky 2 → 46"*. Teda presne to, čo dnes riešite telefonátom. Ak vozíte na viac miest, korekcia bude obsahovať aj **presuny medzi nimi** — počet ostane rovnaký, ale auto sa balí inak.

---

## Ceny a fakturácia

**Cena za jedlo:** ……… € *(ak sa jedlá líšia cenou, uveďte ku každému)*

**Sú ceny s DPH?** ……… *(potrebujeme cenu **bez DPH** a sadzbu zvlášť — príspevok zamestnávateľa sa počíta zo základu)*

**Ako často fakturujete?** ………

**Kedy sa mení cenník?** ……… *(aby sme zmenu zaviedli k správnemu dátumu a nespätne)*

### Časť stravníkov si obed platí sama

Objednávku posielame za všetkých naraz, ale **zaplatiť ju majú dve rôzne strany**. Za zamestnancov platí firma. Časť ľudí u nás ale nie je v pracovnom pomere — tí si svoj obed hradia sami, v plnej cene.

Zvládneme oboje, len musíme vedieť, ktoré:

- [ ] **fakturujeme im priamo** — firemná faktúra bude bez ich porcií *(napíšte, čo k tomu od nás potrebujete: mená, kontakty, IČO)*
- [ ] **fakturujeme všetko firme**, vysporiadanie si spravíte medzi sebou
- [ ] inak: ………

> Prosíme na to odpovedať, aj keď sa to zdá ako naša vec. Aplikácia porovnáva vašu faktúru s počtom porcií, ktorý sama odoslala — a musí vedieť, či na nej tie porcie majú byť. Inak by hlásila rozdiel **každý mesiac** a zakaždým by sa musel odklikávať ručne.

Nech je odpoveď akákoľvek, vám sa nič nemení: objednávka aj rozvoz idú tak ako doteraz. Ide len o to, komu vystavíte papier.

---

## Potvrdzovanie objednávok

V každej objednávke bude odkaz **„Potvrdiť prijatie"** — jedno kliknutie, bez prihlasovania, bez účtu.

Prosíme kliknúť naň vždy, keď objednávku dostanete. Je to jediný spôsob, ako vieme, že sa k vám naozaj dostala. **Ak potvrdenie nepríde do stanoveného času, aplikácia nám ohlási, že vás treba obvolať** — aby sa nestalo, že sa uvarí zlý počet.

Aký čas je pre vás rozumný? ……… *(30 minút, hodina, dve)*
